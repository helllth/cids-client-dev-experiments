...
//            check("GUP_PLANUNGSABSCHNITT");
    ...
    public static boolean checkBean(final CidsBean dbBean) throws Exception {
        ...
        final String first2 = dbBean.toJSONString();
        ...
        final CidsBean jsonBean = CidsBean.createNewCidsBeanFromJSON(first);
        final CidsBean jsonBean2 = CidsBean.createNewCidsBeanFromJSON(first);
        start = System.currentTimeMillis();
        final String jsonBeanJson = jsonBean.toJSONString();
        final String jsonBeanJson2 = jsonBean2.toJSONString();
        dur = System.currentTimeMillis() - start;
        if (!jsonBeanJson.equals(jsonBeanJson2)) {
            System.out.println("BAMM-Deserialization");
            FileUtils.writeStringToFile(new File(
                    FOLDER
                            + dbBean.getCidsObjectKey().replaceAll("/", "")
                            + ".json"),
